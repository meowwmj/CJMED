...
if ($stmt->rowCount() > 0) {
    ...
    echo "<script>window.location.href ='users1.php'</script>";
} else {
    ...
    if(isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if(move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
            ...
            $sql = "INSERT INTO users (name,email,state,username,password,phone,agency_id,address,photo) VALUES (:a,:b,:c,:d,:e,:f,:g,:h,:i)";
            $q = $db->prepare($sql);
        ...
        $sql = "INSERT INTO users (name,email,state,username,password,phone,agency_id,address) VALUES (:a,:b,:c,:d,:e,:f,:g,:h)";
        $q = $db->prepare($sql);
